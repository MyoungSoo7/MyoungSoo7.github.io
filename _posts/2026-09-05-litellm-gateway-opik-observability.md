...
하단에는 Provider별 지출을 합산한 도넛 차트와 표가 있다. 화면 판독 기준 총 지출은 `$0.14`이며, Provider는 `gemini`, `nous`, `openai`로 나뉜다. 표시된 표에는 성공 요청 합계 44건, 실패 0건, 총 토큰 약 228,603개가 보인다. 별도 프로젝트 상세 화면에서는 같은 관측 기간에 총 78건 중 성공 44건·실패 34건으로 표시되므로, 화면별 기간·필터·집계 기준을 확인해야 한다.
...
여기서 `failed = 0`은 이 Provider별 표의 집계 범위에서 실패가 표시되지 않았다는 뜻이지, 전체 프로젝트 실패가 0건이거나 업무 결과가 모두 정확하다는 뜻은 아니다. 별도 프로젝트 상세 화면에는 총 78건 중 34건 실패가 표시되므로, 성공한 API 응답도 잘못된 답변·부적절한 도구 호출·검증 누락을 포함할 수 있다.
...
## 5. 추가 그림: LiteLLM 프로젝트 사용량 상세

![LiteLLM 프로젝트 사용량 상세 화면](/assets/images/litellm-project-spend.jpg)

추가로 확인한 프로젝트 상세 화면은 `2026-08-29 ~ 2026-09-05` 기간을 보여준다. 화면에는 총 지출 `$0.1441`, 제한 없는 예산, 총 요청 78건, 성공 44건, 실패 34건, 요청당 평균 비용 `$0.0018`, 총 토큰 228,603개가 표시된다. 일별 지출은 `2026-09-03`에 약 `$0.14`로 집중되어 있다.

이 화면의 성공률은 `44/78`, 약 56.4%이고 실패율은 약 43.6%다. 이는 프로젝트 상세 화면에 표시된 계산값이며, 실패 원인과 업무 성공 여부는 개별 로그·request ID를 추가로 확인해야 한다. 비용이 낮다고 서비스가 안정적이라는 뜻은 아니다.

## 6. 추가 그림: AI Gateway Logs

![AI Gateway LLM 요청 로그](/assets/images/litellm-ai-gateway-logs.jpg)

추가 로그 화면은 `AI Gateway > Logs`에서 `Sep 4`의 LLM 요청을 행 단위로 보여준다. 각 행에는 시각, 성공·실패 상태, latency, 비용, 토큰 수, 모델명, request 또는 trace 식별자가 표시된다.

화면에는 `openai/gpt-4.1`, `openai/gpt-4.1-mini`, `openai/gpt-4o` 계열과 `gemini/gemini` 계열이 보인다. 예시 토큰은 17~284개 수준의 짧은 요청부터 약 12,900~21,371개 수준의 긴 요청까지 넓게 분포하며, `openai/gpt-4.1`의 한 행은 약 12,920 tokens로 표시된다. 성공 행의 latency는 화면에 보이는 범위에서 약 0.61~5.32이고, 일부 실패 행은 비용·토큰이 `-` 또는 `0`으로 표시된다.

표의 일부 모델명은 화면 폭 때문에 잘려 있고 각 실패의 정확한 원인은 이미지에서 확정할 수 없다. 따라서 이 화면은 “어떤 모델 호출이 언제 발생했고 비용·토큰·지연이 어떻게 기록됐는가”를 보여주는 증거이지, 실패 원인까지 설명하는 완결된 RCA는 아니다.
...
*LiteLLM과 Opik 화면의 수치는 제공된 스크린샷을 판독해 기록한 예시 관측값이다. 화면에 표시된 집계가 전체 운영 시스템과 Provider 청구액을 대표한다고 단정하지 않으며, 이미지의 일부 헤더·모델명은 화면 폭 때문에 잘려 있다. 실제 운영 판단에는 원천 로그·request ID·Provider 청구 데이터·개별 Trace를 함께 대조해야 한다.*
