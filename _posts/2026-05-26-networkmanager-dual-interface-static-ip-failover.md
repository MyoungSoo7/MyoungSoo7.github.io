...
| 단계 | 명령 | 효과 |
|---|---|---|
| 1. 유선·무선 둘 다 static .113 | `nmcli con mod <con> ipv4.method manual ipv4.addresses <lan>.113/24 ipv4.gateway <lan>.1 ipv4.dns "<lan>.1 1.1.1.1"` | DHCP 의존성 제거 |
...
| 4. k3s 가 IP 변경 인지 | `K3S_AGENT_ARGS="--node-ip=<lan>.113 --node-external-ip=<lan>.113"` + `systemctl restart k3s-agent` | Node object 의 InternalIP 가 .113 으로 갱신 |
...
enp2s0           UP   <lan>.116/24 fe80::2d8:61ff:fe2d:51ef/64
wlxb0386cf8d624  UP   <lan>.113/24 fe80::2cae:8073:961e:2d46/64
...
```bash
$ ssh admin@<lan>.116 "ss -tn | grep ':22'"
ESTAB  0  0  <lan>.116:22  <lan>.108:54932
```

`<lan>.116` — 즉 *유선* 으로 들어와 있다. 그러면 *유선 IP 를 바꾸는 명령* 은 *현재 SSH 를 끊는다*. 두 가지 방법:
...
sudo nmcli con mod 'netplan-enp2s0' \
  ipv4.method manual \
  ipv4.addresses <lan>.113/24 \
  ipv4.gateway <lan>.1 \
  ipv4.dns "<lan>.1 1.1.1.1" \
...
```bash
$ ssh admin@<lan>.113 "ip -br addr | head -3"
lo               UNKNOWN   127.0.0.1/8 ::1/128
enp2s0           UP        <lan>.113/24 <lan>.116/24 ...
wlxb0386cf8d624  UP        (IPv6 only)
```

흠 — `<lan>.116` 이 *아직 enp2s0 에 붙어있다*. NetworkManager 가 *새 IP 추가* 만 하고 *이전 DHCP lease 의 IP* 는 해제 안 함. 잔여 IP 라 통신엔 문제 없지만 *깔끔하지 않음*. 정리:

```bash
$ ssh admin@<lan>.113 "sudo ip addr del <lan>.116/24 dev enp2s0"
$ ssh admin@<lan>.113 "ip -br addr show enp2s0"
enp2s0   UP   <lan>.113/24 fe80::...
```
...
```bash
$ ssh admin@<lan>.113 "cat /etc/systemd/system/k3s-agent.service.env"
K3S_URL=https://<control-plane-ip>:6443
K3S_TOKEN=<redacted>
K3S_AGENT_ARGS="--node-ip=<lan>.113 --node-external-ip=<lan>.113"
```
