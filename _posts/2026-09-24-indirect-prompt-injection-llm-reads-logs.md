...
> **정정 (2026-09-25).** 게시 후 코드와 대조해 4곳을 고쳤다. 원문을 조용히 바꾸지 않고 여기에 남긴다.
> 1. ④ 도구 목록 — 원문은 "`es_search`·`kube_read`·`finish` 뿐". 실제로는 `container_lookup` 이 항상 있고 `log_search`·`recovery_check`·`skill_query` 가 설정에 따라 붙는다. 모두 읽기·조회라 "속아도 바꿀 수 있는 게 없다"는 결론은 같다. 단 `skill_query` 는 외부 API 를 부르므로 모델이 쓴 질의문이 클러스터 밖으로 나간다.
> 2. ④ 권한 — 원문은 "get/list/watch". 실제 ClusterRole 은 get/list 뿐이다.
> 3. ⑤ 이그레스 — 원문은 "로그에서 읽은 URL 로 데이터 유출"을 막는다고 적었다. 틀렸다. 외부 443 은 사설망만 빼고 열려 있다(NIM·텔레그램 IP 가 고정이 아니라서). URL 유출을 막는 것은 URL 을 여는 도구가 없다는 설계와 카드 링크 미리보기 끄기다.
> 4. §4-3 "가드는 알림 본문에만" — 게시 다음 날 도구 출력에도 가드를 태우도록 바꿨다. 조사 1회당 상한이 있다.
...
| ④ 도구 계약 | 설계 | 도구는 전부 읽기·조회다 — `es_search`(또는 `log_search`)·`kube_read`·`container_lookup`, 설정에 따라 `recovery_check`·`skill_query` — 그리고 `finish`. 인자는 코드에서 화이트리스트 검증. ServiceAccount 는 get/list 만 | **속아도 바꿀 수 있는 게 없음** |
| ⑤ 이그레스 제한 | 설계 | NetworkPolicy 로 나갈 수 있는 목적지·포트를 허용목록으로. 단 외부 443 은 "사설망 제외 전체" 다 | 내부망 횡이동. URL 유출은 **이그레스가 아니라** URL 을 여는 도구가 없다는 것(④)과 카드의 링크 미리보기 끄기로 막는다 |
...
3. ~~**가드는 알림 본문에만 태운다.**~~ *(2026-09-25 해소)* 이제 로그 검색 결과처럼 자유 텍스트인 도구 출력도, 정규식이 아무것도 못 찾았을 때 가드에 한 번 더 태운다. 다만 지연 때문에 **조사 1회당 횟수 상한**이 있어서, 로그를 많이 읽는 조사에서는 뒤쪽 출력이 정규식만 거친다.
