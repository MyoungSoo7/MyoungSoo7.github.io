...
홈랩 k3s 클러스터의 노드 중 하나는 노트북이다. 서버로 쓰기엔 애매한 기계라고 생각했는데, 뒤집어 보면 **배터리가 달린 유일한 노드**다. 정전이 나면 데스크톱 노드들은 즉시 꺼지지만, 이 노트북은 배터리로 한동안 버틴다.
...
규약상 **방전 중에는 음수**다. 내 첫 구현대로라면 규약을 따르는 드라이버에서는 남은 시간이 영원히 "계산 중" 으로 나온다. 드라이버가 실제로 어떤 부호를 주는지는 방전 중에 직접 찍어 봐야 아는데, 이 글을 쓰는 시점에는 어댑터가 꽂혀 있어 확인하지 못했다. 그래서 부호와 상관없이 동작하도록 **절댓값**을 쓰는 것으로 고쳤다. 가짜 파일로 만든 테스트에서는 양수만 넣었으니, 테스트로는 잡을 수 없었고 문서를 다시 읽어서 잡은 버그다.
