...
*   **기술적 판단**: 시스템 라이브러리(glibc) 제한으로 인해 `/etc/resolv.conf`에 정의된 네임서버 중 상위 3개(`8.8.8.8`, `1.1.1.1`, `[ISP DNS]`)만 적용되고 나머지는 무시되었습니다. 이는 서비스 장애로 직결되지는 않으나, 업스트림 DNS 쿼리 시 예기치 않은 지연이나 실패를 유발할 수 있는 잠재적 리스크입니다.
