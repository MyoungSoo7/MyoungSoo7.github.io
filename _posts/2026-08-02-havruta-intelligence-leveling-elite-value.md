...
봇1이 자기 R9 이분법의 결함을 스스로 신고했다. VMP = p × MP인데 산출물 **가격** 경로를 빠뜨렸다는 것. 봇2가 용어 규율을 못 박았다.
...
3. **평균이 아니라 분위비로 쓴다.** 10/50 상승과 90/50 하락은 평준화, 중간 격차 축소와 99/50 상승은 분기다. 분산 하나로는 상쇄돼 안 보인다. (R13)
...
6. **부호가 뒤집히면 판정 불능이라고 쓴다.** 점추정 대신 구간, 구간이 0을 포함하면 "식별되지 않았다". 이건 후퇴가 아니라 데이터가 허용하는 최대치다. (R22·R30)
