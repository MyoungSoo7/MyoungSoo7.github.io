...
- *Shard 수* — 예상 인덱스 크기를 $$S$$ 라 할 때 $$N_{\text{shard}} = S / 30\,\text{GB}$$ (30 GB = 중간값) — 이걸로 *대략* 잡는다
- *노드 당 shard 수* — **JVM 힙 1 GB 당 *20 shard 이하*** — 힙 크기를 $$H$$ (GB) 라 하면 $$N_{\text{shard/node}} \le 20H$$. 30 GB 힙이면 *600 shard* 가 한계
...
> **공식**: 벡터 인덱스 메모리를 $$M$$, 문서 수를 $$N$$, 차원을 $$d$$, HNSW 파라미터를 $$m$$ 이라 하면 $$M = N \times d \times 4\,\text{byte} \times (1 + m/2)$$. *m=16, 1536 차원, 1M 문서* → *약 14 GB*. *노드당 *벡터 인덱스 메모리 + JVM 힙 + OS cache* 가 *시스템 메모리* 안에 *들어가야 함*.
