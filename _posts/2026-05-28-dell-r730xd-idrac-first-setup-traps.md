...
| 5 | 관리 PC 가 *모뎀 직결* 이라 사설망 못 봄 | iDRAC `<mgmt>.106` 접속 timeout, 본인 IP 는 `203.0.113.x` (공인 IP) | 데스크탑 LAN 케이블 → *공유기 LAN 포트* 로 이동 (모뎀 직결 X) |
...
```bash
$ ping -c 5 <mgmt>.0/24 의 가능한 IP 들  # 또는 router admin 페이지에서 DHCP 리스트
$ arp -a | grep -i "10:98:36"      # Dell OUI
? (<mgmt>.106) at <mac-redacted> on en0 ifscope [ethernet]  ← iDRAC IP 찾았다!

$ curl -sk https://<mgmt>.106/
HTTP/1.1 302 Found
...
iDRAC `<mgmt>.106` 가 *분명히 살아있는데* 데스크탑 브라우저:

```
사이트에 연결할 수 없음
<mgmt>.106에서 응답하는 데 시간이 너무 오래 걸립니다.
...
[ISP 모뎀] ─┬── 데스크탑 (203.0.113.45)  ← 직결, 공인 IP
            │
            └── [공유기] ── 노트북 (WiFi) / iDRAC (LAN)
                 <mgmt>.1     (<mgmt>.x 사설망)
```

데스크탑은 *공인 IP* 를 직접 받아서 사설망 `<mgmt>.x` 와 *완전히 다른 네트워크* — 서로 통신 불가. 노트북은 WiFi 로 공유기에 붙어있어서 iDRAC 잘 보였던 것.
...
```
[ISP 모뎀] ─── [공유기] ─┬── 데스크탑 (LAN, <mgmt>.115)
                 <mgmt>.1 ├── 노트북 (WiFi, <mgmt>.x)
                          └── iDRAC (LAN, <mgmt>.106)
```

1. 데스크탑의 LAN 케이블을 *모뎀이 아니라 공유기 LAN 포트* 로 옮김
2. 데스크탑에서 `ipconfig /release && ipconfig /renew`
3. 새 IPv4 가 `<mgmt>.xxx` 로 잡힘 → iDRAC 접속 가능 + NAT 보호도 얻음
...
$ ping <mgmt>.106
64 bytes from <mgmt>.106: time=2.78 ms    ← 응답 ✅
0% packet loss
