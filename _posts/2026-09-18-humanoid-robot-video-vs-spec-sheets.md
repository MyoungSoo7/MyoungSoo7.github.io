...
문제는 이 질문에 영상이 답을 못 한다는 것이다. 영상은 성공한 테이크만 남긴다. 실패한 테이크도, 사람이 뒤에서 조종하고 있었는지도, 배터리가 몇 분 남았는지도 프레임 안에 없다. 그래서 이 글은 영상을 해설하지 않는다. **영상으로는 판정이 안 된다는 것을 먼저 확인하고, 판정이 가능한 자료 — 제조사 공식 사양서와 대회 참가팀의 자체 보고서를 모은 학술 문헌 — 으로 갈아타는 과정**을 그대로 적는다.
...
이 문제의 가장 좋은 1차 문헌은 2015년 DARPA Robotics Challenge(DRC) 결선 직후에 나왔다. 참가팀들이 자발적으로 제출한 자체 보고서를 모아 분석한 Atkeson 외, *"What Happened at the DARPA Robotics Challenge, and Why?"* 다(같은 저자들의 학회 논문을 확장한 판본이다). 카네기멜론·WPI 연구팀이 직접 참가하면서 쓴 글이라, 실패를 밖에서 관찰한 게 아니라 **당사자로서 기록한** 자료다.
